...
int		pg(t_game *game, char *path)
{
	...
	if (ft_endwith(path, ".cub") == 0)
		exitit_2(" not valid map :  please specify --save and .cub", game);
	if ((fd = open(path, O_RDONLY)) < 0)
		exitit_2(" can't open the file ", game);
...
void	makewindow(t_game *g, int screen, char *path)
{
	...
	init_game(g);
	if (pg(g, path) == 0)
		exitit_2("error while parsing the file", g);
	else
	{
		init_game_2(g);
		g->img = mlx_new_image(g->mlx, g->w, g->h);
		...
		g->imgpoke = mlx_get_data_addr(g->img, &g->bpp, &(g->sl), &(g->end));
		if (!(g->zbuffer = (double*)malloc(sizeof(double) * g->w)))
			exitit("error malloc gzbuffer \n", g);
		raycast_procjection(g);
		mlx_put_image_to_window(g->mlx, g->win, g->img, 0, 0);
		player(g);
		if (screen == 1)
